...
def save_config():
    """Save config.yaml content for a node working directory."""
    try:
        data = request.get_json(silent=True) or {}
        working_directory = str(data.get('working_directory') or '').strip()
        filename = str(data.get('filename') or 'config.yaml').strip()
        ...
        base_path = SCRIPT_DIR.parent.parent.resolve()  # webroot
        target_dir = (base_path / sanitized_dir).resolve()
